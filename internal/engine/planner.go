...
func projectPlannerPrompt(skills []string, executionContext projectPlannerExecutionContext, repository, idea string) string {
	...
	b.WriteString("\nRetain exact accepted references and established shared-contract or verification-environment constraints in the affected card details. Historical ideas are not accepted requirements. Add integration/readiness work only for distinct required proof no delivery card can establish; name that gap rather than repeating completed checks. Keep host-only proof and its approved handoff distinct from sandbox work. A changed candidate needs a new bound evidence record; applicable underlying checks may be reused with explicit justification and delta proof, not relabelled as a new whole-candidate run. Do not invent a browser, deployment, or other interface requirement.")
