...
func TestLiveCodexGoDevelopmentHarness(t *testing.T) {
	...
	instructions := "Use the runner-implementer skill. Work only in the assigned worktree. " +
		...
		"Create a small Go main module named example.com/runner-go-live that imports rsc.io/quote at v1.5.2, with a focused test asserting that quote.Hello() returns a non-empty string; do not assert a locale-specific greeting or change the locale. Use the installed go executable to resolve the public dependency, then run go vet ./..., go test ./..., go build ./..., and go mod verify. " +
