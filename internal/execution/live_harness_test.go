...
func TestLiveReviewerBrowserHarness(t *testing.T) {
	...
	for _, kind := range strings.Split(requested, ",") {
		...
		t.Run(kind, func(t *testing.T) {
			...
			browserInstruction := "Use only the navigate and evaluate tools in Runner's runner_browser MCP server. Call navigate directly; do not inspect list_mcp_resources because it does not list tools. Do not launch a browser through shell commands."
