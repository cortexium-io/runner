...
Follow the reviewer skill's verification scheduling: run heavyweight commands sequentially within this assignment, including test suites, browser runs, builds, and dependency installation. Do not overlap them through parallel tool calls or background jobs. Keep each command's configured workers and timeouts; a required application server may remain running for its active check. Record any correction of accidental overlap separately from unchanged failure reproduction.
