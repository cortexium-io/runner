...
| `Blocked` | Human: input, an execution error, a closed-without-merge PR, or the QA retry budget requires attention. |
...
6. A QA rejection moves the card back to `Ready` and increments its rejection
   counter. With the generated `max_qa_retries` of 3, Runner permits three
   rework-and-review retries after the initial rejection; a fourth consecutive
...
cannot exceed the initial implementation plus the reviewer lane's
`max_qa_retries`, because a longer ladder would contain unreachable profiles.
...
uses `implementer_luna`, then `implementer_sol`. If the QA retry budget permits
...
- Reviewer lanes additionally define `max_qa_retries`, `rejected`, and
  `exhausted`.
