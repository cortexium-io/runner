...
func runRetry(ctx context.Context, args []string, stdin io.Reader, stdout io.Writer) error {
	...
	feedback := flags.String("feedback", "", "replace stale retry feedback and reset QA failures; does not amend approved requirements (use amend)")
	reauthorize := flags.Bool("reauthorize", false, "review and reauthorize retained implementation or published human rework in assessment; requires terminal confirmation; --json previews only")
